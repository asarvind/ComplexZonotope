...
\item \plemph{trajectory set} computation for \plemph{verificationxsxs}
\end{itemize}
\end{block}
%
%\emph{}
...
\begin{frame}{Hybrid Dynamics of Embedded/Cyber-physical Systems}
...
\end{block}
%
\negemph{*} We \negemph{can not exploit} the above proposition when \negemph{eigenvectors are complex}.
\end{frame}
...
\node at (2.25,0) {$\cztope{\mymatrix{V & W}}{c}$};
...
\begin{frame}{Intersection between Template Complex Zonotopes}
%
\begin{itemize}
...
\hline
\multirow{2}{*}{{\bf Set}} & \multirow{2}{*}{{\bf Linear}} & \multirow{2}{*}{{\bf Minkowski}} & \multirow{2}{*}{{\bf Intersection} } & {{\bf Positive Invariant} }\\
\multirow{2}{*}{{\bf representation}} & \multirow{2}{*}{{\bf transformation}} & \multirow{2}{*}{{\bf sum}} & \multirow{2}{*}{{\bf with half-space}} & \\
& & & & stable invertible\\
...
\begin{itemize}
\item Secondary template is \plemph{pseudo-inverse} of parallelotopic template of \plemph{guards}.
...
\item Controller input has \textcol{saturation}: \textcol{hybrid behavior}.
\end{itemize}
\end{minipage}
