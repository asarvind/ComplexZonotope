...
particular case when zonotopes are closed under intersection with
sub-level sets of linear inequalities.  For explaining our observation
succinctly, we introduce a slight variation of the representation of a
real zonotope, called an \emph{interval zonotope}.  In an interval
zonotope, we specify interval bounds on the combining coefficients,
without explicitly specifying the center.  By such a representation,
we can express the resultant interval zonotope of the aforementioned
intersection by a simple algebraic expression.  However, we note that
an interval zonotope is geometrically the same as a real zonotope.
%
...
\end{definition}
%
We consider a particular class of sub-level sets of linear
inequalities, which we call \emph{sub-parallelotopes}, for which the
...
vectors $\lt[\begin{matrix}1 & 1 & -1 \end{matrix}\rt]$ are linearly
...
\end{align*}
%
is a sub-parallelotope specified as
%
\[
...
\end{proof}
%
The above similarity between sub-parallelotopes and an interval
zonotopes gives an apparent intuition that interval zonotopes can be
closed under intersection with suitably aligned
sub-parallelotopes.  We observe that when a
...
\end{proof}
%
We shall derive an over-approximation of intersection of a
...
about convex sets which is used in the deriving the over-approximation.
...
We use the following notation.  If $k<n$, then the column vectors of a
matrix $Y\in\mat{n}{(n-k)}{\reals}$ comprises the basis of $\null{\qtemp}$.
If $k=n$, then we consider $Y=0$.
%
...
\]
%
Then, Equation~\ref{eqn:intersection1} follows from the above two results.

Now we shall prove Equation~\ref{eqn:intersection2}.

Let us consider
\begin{align*}
& x\in \lt(S\oplus \iztope{\pinv{\qtemp}}{\lb}{\ub}\rt)\bigcap\ptope{\qtemp}{\plb}{\pub}~~\text{where}\\
...
$\min\set{\ub_i,\pub_i}=\pub_i$.  By using
Equation~\ref{proof-intersection1}, we get
$\qtemp_iw=\qtemp_iv+\zeta_i-\epsilon_i\leq\pub_i-\min\set{\ub_i,\pub_i}=\pub_i-\pub_i=0$.
Also by using Equation~\ref{proofnew-intersection1}, we get
$\qtemp_iw=\qtemp_iv+\zeta_i-\epsilon_i\geq \qtemp_iv+\min\set{\ub_i,\pub_i}-\min\set{\ub_i,\pub_i}=\qtemp
v_i.$  Therefore,
%
\begin{align*}
& \qtemp_iw\in\convexhull{\set{0,\qtemp_iv}}.~\numberthis\label{proofnew-intersection2}
\end{align*}
%
{\it Case 2:} Let us consider that $\zeta_i<\max\set{\lb_i,\plb_i}$.
Then $\epsilon_i=\max{\set{\lb_i,\plb_i}}$.  Since $\zeta_i\geq\lb_i$,
we get $\max{\set{\lb_i,\plb_i}}=\plb_i$.  By using
Equation~\ref{proof-intersection1}, we get
$\qtemp_iw=\qtemp_iv+\zeta_i-\epsilon_i\geq\plb_i-\min\set{\ub_i,\pub_i}=\plb_i-\plb_i=0$.
Also by using Equation~\ref{proofnew-intersection1}, we get
$\qtemp_iw=\qtemp_iv+\zeta_i-\epsilon_i\leq \qtemp_iv+\max\set{\lb_i,\plb_i}-\max\set{\lb_i,\plb_i}=\qtemp
v_i$.  Therefore,
%
\begin{align*}
...
\end{align*}
%
From Equations~\ref{proofnew-intersection2}-\ref{proofnew-intersection4}, we
get
%
\begin{align*}
...
\end{align*}
%
As $S$ is a convex set and $v\in S$, by Lemma~\ref{lem:convexhull} and
Equations~\ref{eqn:intersection3} and~\ref{proofnew-intersection5}, we
get $\qtemp w\in\qtemp S$.  As $Y^T$ is orthogonal to $\qtemp$, we get
$Y^Tw=Yv+Y\qtemp(\zeta-\epsilon)=Y^Tv+0=Y^Tv\in Y^TS$ .  So,
%
\begin{align*}
& \mymatrix{\qtemp\\ Y^T}w\in S.
\end{align*}
%
By
