...
%
\begin{proof}
  By Lemma~\ref{lem:ptope-iz-conversion}, we get
  %
  \begin{align*}
  & \supp{v}{\ptope{\qtemp}{\plb}{\pub}}=\supp{v}{\nullspace{\qtemp}}\oplus\supp{v}{\Psi}~~\text{where},\\
& \Psi=\set{\zeta\in\reals^k:~\plb\leq\zeta\leq\pub}.
  \end{align*}
%
As $\nullspace{\qtemp}$ is a vector space, so if
$\supp{v}{\nullspace{\qtemp}}\neq 0$, then
...
%
\begin{align*}
&\supp{v}{\ptope{\qtemp}{\plb}{\pub}}=\supp{v}{\Psi}\setminus{\infty}\\
&
  \%\% ~~\text{since support of a function can not be} -\infty
  \nonumber\\
& = \supp{v}{\Psi}\bigcap\reals
=
  \lt(\max_{x\in\Psi}v^Tx\rt)\bigcap\reals=\lt(\max_{\zeta:\plb\leq\zeta\leq\pub}v^T\pinv{\qtemp}\zeta\rt)\bigcap\reals\nonumber\\
&
  =
  \sum_{i=1}^k\lt(\max\lt(\lt(v^T\pinv\qtemp\rt)_i\set{\plb_i,\pub_i}\rt)\bigcap\reals\rt).\nonumber\\
&
  \subseteq\bigoplus_{i=1}^k
  \lt(v^T\pinv{\qtemp}\rt)\lt(\set{\plb_i,\pub_i}\bigcap\reals\rt)\nonumber\\
&
  \%\% ~~\text{From the definition of}\nonumber\\
& \%\%
  ~~\text{min-approximation and max-approximation
    functions},\nonumber\\
& \subseteq
  \bigoplus_{i=1}^k\lt(v^T\pinv{\qtemp}\rt)_i\lt(\set{\lt(\maxapprox{\lb}{\plb}\rt)_i,\lt(\minapprox{\ub}{\pub}\rt)_i}\rt)\nonumber\\
&
  \subseteq
  v^T\pinv{\qtemp}\set{\zeta\in\reals^k:\zeta_i\in\set{\lt(\maxapprox{\lb}{\plb}\rt)_i,\lt(\minapprox{\ub}{\pub}\rt)_i}}\nonumber\\
&
  \%\%~~\text{since}~\lb\leq\maxapprox{\lb}{\plb}\leq\minapprox{\ub}{\pub}\leq\ub\nonumber\\
&
  \subseteq v^T\pinv{\qtemp}\set{\zeta\in\reals^k:\lb\leq
    \zeta\leq\ub}\nonumber\\ & = v^T\iztope{\pinv{\qtemp}}{\lb}{\ub}.
\end{align*}
%
Therefore,
%
\[
\exists
x\in\iztope{\pinv{\qtemp}}{\lb}{\ub}:\supp{v}{\ptope{\qtemp}{\plb}{\pub}}=\supp{v}{\Psi}=v^Tx.\vspace{3em}~\qedhere
\]
%
\end{proof}
