...
relation.
%
\begin{definition}[A relation between template complex zonotopes]
...
%
\begin{proof}
The R.H.S of Equation~\ref{eqn:transfer-matrix} is an upper bound on
the L.H.S of Equation~\ref{eqn:exact-inclusion} based on
Lemma~\ref{lem:transfer-matrix}.  This means that if the R.H.S of
Equation~\ref{eqn:transfer-matrix} is bounded above by zero, then
the condition for inclusion in Equation~\ref{eqn:exact-inclusion} is
satisfied.
\end{proof}
%
...
then $\tcz{\ptemp}{\cen}{\sfact}\order\tcz{\ptemp^\dpr}{\cen^\dpr}{\sfact^\dpr}$.
\end{enumerate}
%
\end{theorem}
%
...
and
$\tcz{\ptemp^\pr}{\cen^\pr}{\sfact^\pr}\order\tcz{\ptemp}{\cen}{\sfact}$.
By Theorem~\ref{thm:suff-inclusion}, this also means
...
that}\\
& \ptemp^\pr\tmat=\ptemp^\dpr\diagonal{\sfact^\dpr},~~\ptemp\tvect=\cen^\dpr-\cen^\pr,\\
