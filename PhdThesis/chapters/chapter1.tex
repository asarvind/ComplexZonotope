...
0}^n:\Psi\subseteq\tcztope{\ptemp}{\cen}{\sfact}}$.  Then $\sfact\in S$ if
and only if
%
\begin{equation}
\forall i\in\set{1,...,n},~\max_{x\in\Psi}\lt(\inv{\ptemp}x\rt)_i\leq \sfact_i.
\end{equation}
%
Consequently, if $\sfact^*=\vectormin{s\in S} s$, then
%
\begin{equation}
\Psi\subseteq\tcztope{\ptemp}{\cen}{\sfact^*}=\bigcap_{\sfact\in S}\tcztope{\ptemp}{\cen}{\sfact}.
\end{equation}
%
\end{theorem}
%
