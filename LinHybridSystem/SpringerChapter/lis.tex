...
template complex zonotopes.  %% Our motivation of for considering
%% template complex zonotopes is as follows.  For a fixed sampling
%% period, the contraction of a template complex zonotope having
%% eigenvectors of the reachability operator in the template is bounded
%% by the largest absolute value of the eigenvalues.
%% %% Before discussing our motivation for
%% %% considering template complex zonotopes, we state the following
%% %% proposition required to explain our motivation.
%% %% %
%% %% \begin{prop}
...
\vspace{0.5em} \emph{Synthesizing a candidate template complex
...
\emph{Verifying contraction.}  To verify that this
synthesized template complex zonotope actually contracts with respect
...
bounding such contraction gives the following result.
 %%  Let $H_{t+\rho}$ be an operator where $\rho$
%% lies in the interval $(0,\epsilon)$.  Then it was shown
%% in~\cite{arvind2016lis} using Taylor expansion that \[H_{t+\rho} =
%% P_r^t(\rho) + E_r^t(\delta),\] where
%% %
%% $ P^t_r(\rho)=\sum_{i=0}^r\frac{A_c^i\rho^i}{i!}H_t$ and $
%% E^t_r(\delta)=\frac{A_c^{r+1}\delta^{r+1}}{(r+1)!}H_t $ for an order
%% of expansion $r\in\pint$ and some $\delta\in[0,\epsilon]$.
...
%% \emph{Algorithm to verify contraction.}  We first choose a minimum
%% tolerance value, say $tol$ and an order of expansion
%% $r\in\mb{Z}_{>0}$.  Starting with $t=t_{min}$ and for some $h\geq
%% tol$, we check whether $\eta_r(V,s,t,h) < 1$.  If this condition is
%% satisfied, $t$ is incremented to $t+h$ and $h$ is incremented to
%% $h+tol$.  Then the same is repeated. If the condition is not
%% satisfied, then $h$ is reduced to $h-tol$ and we repeat the
%% procedure. In this process, if we hit a value of $t$ greater than
%% $t_{max}$, this means that we have verified contraction with respect
%% to all the reachability operators.  Otherwise, if we get $h<tol$, then
%% the process is terminated.  In the latter case case, we can increase
%% the value of $k$ to have a larger template and repeat the procedure,
%% as described below.
