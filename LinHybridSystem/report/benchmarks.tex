...
transformed state of the composite system,
$\trj{y}{t}\in\realset^2$ is the input sent by the
controller, $\trj{u}{t}\in\realset^4$ is the additive disturbance
input and $sat$ is the saturation function which limits
...
i\in\{1,2\}$, where $\delta=100$ and $d_p=0.0807$.  For the
...
The state space of the satruated system can be divided into nine
...
the edges.  The unsaturated system is modeled with one location and no
...
\tbf{Implementation.}  The primary template for the hybrid system is
...
in our approach we use the complex eigenstructure, we could find the
...
\tbf{Implementation}.  For the primary template, we collected the
...
\tbf{Size of slow switching model}: 9 dimensional, 2
locations and 4 edges.

\tbf{Size of fast switching (integer times) model}: 9 dimensional, 2
locations, 2 edges.
