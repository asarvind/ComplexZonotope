classdef Invariant1
    ...
    methods (Static)
        ...
        function [ retval ] = ConstraintACZtope( tag,scalename,upperbound,lowerbound )
            ...
            txt2 = sprintf('[(%s); ((%s)-(%s))/2]+epsilon;',scalename,upperbound,lowerbound);
            retval = [txt1,txt2];
        end
    ...
    methods
        ...
        function this = SynTemplate(this,system)
            for i = 1:this.system.NumLocs
                if ~isempty(this.system.Ptemplate{i})
                    this.SecTemplate{i} = pinv(this.system.Ptemplate{i});
                    this.PrimTemplate{i} = system.V{i};
                else
                    error('no paralletopic template specified')
                end
            end
        end
        
        function [ this ] = Invariant1( system,filename,type,solver,precision )
            clear -global
            this.system = system;
            this.filename = filename;
            
            % Synthesize template
            this = this.SynTemplate(system);
            ...
            for i = 1:numel(H.edges)
                ...
                fprintf( fid,'\n variables Xreal_edge%s(%s,%s) yreal_edge%s(%s,1)',num2str(i),num2str(m3+m4),num2str(m1+m2+m5+m6),num2str(i),num2str(m3+m4) );
                fprintf( fid,'\n variables Xcomp_edge%s(%s,%s) ycomp_edge%s(%s,1)',num2str(i),num2str(m3+m4),num2str(m1+m2+m5+m6),num2str(i),num2str(m3+m4) );
            ...
            % specify inclusion relations and center constranints for intralocation dynamics
            for i = 1:H.NumLocs
                clear {'acz1','acz2','tag','txt'};
 
                fprintf( fid, '\n transpose(W{%s})*c_%s == 0 \n',num2str(i),num2str(i) );
                ...
                fprintf( fid,'\n %s invariance after transition in location %s',comment, num2str(i) );
%                 fprintf( fid,'\n upr_loc%s <= u_%s',num2str(i),num2str(i) );
%                 fprintf( fid,'\n l_%s <= lpr_loc%s',num2str(i),num2str(i) );
            ...
            for i = 1:numel(H.edges)
                if ~isempty(H.edges{i})
                    edge = H.edges{i};

                    clear {'acz1','acz2','tag','txt'};

                    % Bounds on variables
    %                 fprintf( fid,'\n lpr_edge%s <= upr_edge%s \n',num2str(i),num2str(i) );

                    % specify acz1 names
                    acz1.PrimTempname = sprintf( 'V{%s}',num2str(edge.loc2) );
                    acz1.SecTempname = sprintf( 'W{%s}',num2str(edge.loc2) );
                    acz1.centername = sprintf( '(edge.type)*c_%s',num2str(edge.loc2) );
                    acz1.scalename = sprintf( '(edge.lambda)*s_%s',num2str(edge.loc2) );
